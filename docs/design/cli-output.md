# TrailTool CLI Output Redesign: Shared Render System and Package Refactor
...
`cmd/trailtool/main.go` with no shared rendering layer: seven independent
`tabwriter` blocks, no terminal awareness, no ANSI styling, inconsistent
capitalization, separators, and timestamps, an alphabetically-sorted "Top Events"
...
into `cli/commands/` (Cobra plus store) and `cli/view/` (formatting) on top of it.
...
- **Color and Unicode are additive.** Every signal (status, denied activity,
  lineage, session type) stays legible in plain ASCII with color stripped. Golden
  tests prove it (§4.2, §7, §9).
- **JSON is a hard contract.** `--format json` stays pure machine-readable JSON
  on stdout: no ANSI, no human prefixes, identical field names, shapes, and
  values. Tests guard this (§8, §9).
- **Layered, testable packages.** `internal/render` imports neither AWS nor
  `core/models`; `cli/view` never touches the store. This seam makes rendering
  testable without AWS, mirroring the ingestor's `lib/<domain>/` split (§3).
- **Refactor first, then style.** The behavior-preserving package split ships as
  ...
  legible and regressions bisect cleanly to "move" versus "restyle" (§10).
...
- **Client counts are already reconciled.** The client line renders
  `N requests: M ok, K denied`. "requests" is inclusive of denied, kept distinct
  from the session's success-only Events line, so no client appears to have more
  events than its session. The redesign keeps this framing (§5.1).
...
The gaps this design targets: no shared render system, no color/TTY/width
awareness, inconsistent IA/typography/timestamps, alphabetically-sorted "Top
Events", leaking raw AWS errors, and stdout/stderr interleaving in `status`.
...
cli/
  commands/          # Cobra glue and store access; one file per command group
...
`internal/render` (rather than `cli/render`) because it is a general terminal
primitive with no domain meaning, and `internal/` prevents it becoming an
...
Color is never the only signal. Denied counts also carry the word "denied" and a
symbol; status also carries `[ok]`/`[warn]`/`[fail]`.

### 4.3 Symbol vocabulary and ASCII fallback
...
- **Sections:** a blank line then a Title-Case heading (`Clients (3):`), body
  indented two spaces. No ASCII rules or separators (`--- … ---`). The blank line
  plus heading is the separator.
...
- **Root/command help:** Cobra-generated, left as-is structurally; we ensure our
  result output's tone and casing match so help and results read as one tool.
...
- **AWS/service errors:** one-line human `Error` plus a one-line hint to stderr;
...
| Redirected stdout | off | Unicode ok, color off | 80 | pipe-clean, stable |
...
| Partial status failure | mixed | both `✓` and `✗` | – | detail to stderr, ordered |
...
- **Golden/snapshot tests** in `cli/view`: feed fixture `models.*`, assert
  ...
  for: one client; mixed client families; version/platform drift; agent and AWS MCP
...
- **No-color parity test:** strip ANSI from a colored render and assert it equals
  the `Color:false` render, proving color is additive.
